...
class vtu:
    ...
    def GetDistance(self, x, y):
        ...
        return math.sqrt(
            sum([(posx[i] - posy[i])**2 for i in range(len(posx))]))
    ...
    def GetField(self, name):
        ...
        if len(self.invalidNodes) > 0:
            ...
            if oldField is None:
                ...
                if oldField is None:
                    raise Exception(
                        "ERROR: couldn't find point or cell field data with name " + name + ".")
            components = oldField.GetNumberOfComponents()
            for invalidNode, nearest in self.invalidNodes:
                for comp in range(nc):
                    array[invalidNode * nc +
                          comp] = oldField.GetValue(nearest * nc + comp)
...
def VtuMatchLocationsArbitrary(vtu1, vtu2, tolerance=1.0e-6):
    ...
    return numpy.allclose(
        locations1[sort_index1],
        locations2[sort_index2],
        atol=tolerance)
...
def VtuDiff(vtu1, vtu2, filename=None):
    ...
    fieldNames2 = [
        vtkdata.GetArrayName(i) for i in range(
            vtkdata.GetNumberOfArrays())]
    if useProbe:
        # meshes are different - we can't interpolate cell-based fields so
        # let's just remove them from the output
        for fieldName in fieldNames1:
