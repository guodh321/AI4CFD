...
Description: this is the python script to implement the ventilation control using PredAAE and doing 4D-Var DA 
...
train_ct_all = joblib.load(root_path + 'Cotrace_fixed_720_cases/caseALL_control/control_all_train_ct.pkl')
...
def optimize_coding_aae_sensorvp(initial_condition, real_coding, epochs):
    ...
    def opt_step(initial_condition, real_coding):
        with tf.GradientTape() as tape:
            ...
            # obtain the 3 fields
            real_coding_sensor_co2 = tf.reshape(real_coding, (1, input_timestamps, ncoeffs, 1))[:,:,120:138, :]
            gen_output_sensor_co2 = gen_output[:,:,120:138, :]

            real_coding_sensor_th = tf.reshape(real_coding, (1, input_timestamps, ncoeffs, 1))[:,:,192:228, :]
            gen_output_sensor_th = gen_output[:,:,192:228, :]

            real_coding_sensor = tf.concat([real_coding_sensor_co2, real_coding_sensor_th], axis = 2)
            gen_output_sensor = tf.concat([gen_output_sensor_co2, gen_output_sensor_th], axis = 2)
...
for c in range(1, cases_num+1):
    print('control case{} in progress'.format(c))
...
np.save(root_path + 'Cotrace_fixed_720_cases/caseALL_control/aae_control_nprediction_caseALL_3f.npy', X_predict_all_list)
np.save(root_path + 'Cotrace_fixed_720_cases/caseALL_control/aae_control_4dvar_caseALL_3f.npy', X_predict_4dVar_all_list)